...
export const verifyWarrantyClaim = catchAsyncError(async (req, res, next) => {
  const { name, email, phone, serialNumber, firebaseIdToken } = req.body;

  if (!firebaseIdToken || !serialNumber) {
  ...
  try {
    ...
    if (!verifiedPhoneNumber) {
      return next(new AppError("Phone number not verified", 401));
    }

    // Optional: Ensure frontend phone matches verified phone
    ...
    const customerWarranty = await CustomerWarranty.create({
      customerName: name,
      email,
